...
This document contains the detailed operational guidance for the repository's
tools: the complete script index, cloud workflow, environment profiles,
benchmarks, dataset preparation, local utilities, cost controls, and
troubleshooting. For task selection and safe default behavior, start with
[README.md](README.md).

## Script index and effects

| Script | Role | Typical environment | Main effect |
|---|---|---|---|
| `cloud_gan.sh` | Main entry point for provisioning, validation, detached GAN training, monitoring, and result download | WSL/local host and cloud GPU host | Mutates the remote host for `provision` and writes/downloads training artifacts |
| `setup_cloud_gpu.sh` | Prepare the locked project environment on an existing Ubuntu GPU host | Cloud GPU host | Installs missing host prerequisites and creates/updates the project environment |
| `test_gan_concurrency.sh` | Compare sequential and parallel BF16 GAN workloads | Supported cloud GPU | Consumes GPU time and writes a new benchmark directory |
| `benchmark_h100_precision.sh` | Compare strict FP32, TF32, and BF16 with one fixed workload | NVIDIA H100 | Consumes GPU time and writes a new benchmark directory |
| `benchmark_stylegan_gpu.sh` | Run or compare fixed-workload single-GPU StyleGAN measurements | CUDA GPU | Runs a benchmark or reads two completed TSV files for comparison |
| `benchmark_gan_training.py` | Internal fixed-burst benchmark worker used by the shell benchmark scripts | CUDA GPU | Writes one timing result when invoked by a wrapper |
| `download_dataset.py` | Download and prepare one lesson dataset or the complete collection | Any project host | Downloads data and may build derived dataset caches |
| `plot_fashion_mnist.py` | Save grayscale and binarized Fashion-MNIST sample grids | Any project host | Downloads Fashion-MNIST when absent and writes image files |
| `pytorch_test.py` | Print the installed PyTorch and visible CUDA device information | Any project host | Read-only runtime inspection |
| `sgd_animation.py` | Render the standalone SGD landscape animation | Any project host | Writes `output/sgd_animation.gif` |
| `word_frequency.py` | Plot word counts for the prepared *Time Machine* dataset | Any project host | Writes `output/word_frequency.png` and may open a plot window |
...
| `--dataset` value | Source and dependency profile | Result |
|---|---|---|
| `mnist`, `fashion-mnist`, `cifar10` | torchvision; base environment | Prepared torchvision datasets below `data/` |
| `time-machine`, `airfoil`, `fra-eng`, `pokemon` | Project download registry; base environment | Downloaded lesson assets below `data/` |
| `house-prices` | Project download registry plus `pandas`; `examples` extra | Training and test CSV files below `data/` |
| `celeba`, `anime-face`, `glasses` | Kaggle; `celeba` or `examples` extra | Downloaded datasets and, where required, validated/derived image trees |
...
authentication. Use `--prepare-celeba-cyclegan` whenever CelebA is selected to
also build the black-hair/blond-hair CycleGAN split; `--dataset all` enables
that split preparation automatically.
...
path options. By default it writes `output/fmnist_labels.png` and
`output/fmnist_labels_binary.png`. The SGD animation and word-frequency scripts
currently use fixed output paths under `output/`.
