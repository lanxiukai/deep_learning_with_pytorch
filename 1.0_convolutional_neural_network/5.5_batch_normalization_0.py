...
def batch_norm(X, gamma, beta, moving_mean, moving_var, eps, momentum):
    ...
    if not torch.is_grad_enabled():
        ...
        # Update moving (running) mean and variance
...
class BatchNorm(nn.Module):
    # num_features: number of outputs for a fully connected layer or number of output channels for a conv layer.
    # num_dims: 2 for fully connected, 4 for convolutional layer
    def __init__(self, num_features, num_dims):
        ...
        # Non-parameter buffers for running mean and variance, initialized to 0 and 1
        self.moving_mean = torch.zeros(shape)
        self.moving_var = torch.ones(shape)

    def forward(self, X):
        # If X is not on the same device, move moving_mean and moving_var to the device where X resides
