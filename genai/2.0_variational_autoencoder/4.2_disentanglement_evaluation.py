"""Evaluate beta-VAE and beta-TCVAE on known factors and matched rates.

The report keeps five questions separate:

* distortion and actual rate;
* active latent coordinates;
* factor informativeness through one fixed-capacity supervised probe;
* MIG compactness and per-latent modularity;
* variation across seeds and nearest-rate method pairs.
...
def encode_dataset(
    ...
    *,
    ...
    return all_codes, torch.cat(factors), {
        ...
        "rate": float(all_rates.sum(dim=1).mean()),
...
def aggregate_records(
    ...
    for record in records:
        groups[
            (str(record["model_name"]), float(record["beta"]))
        ].append(record)
    aggregate = []
    numeric_keys = (
        "distortion",
    ...
    for (model_name, beta), group in sorted(groups.items()):
...
def match_rates(
    ...
    for tc_record in tc_records:
        ...
        matches.append(
            {
                "seed": tc_record["seed"],
                "beta_tc_beta": tc_record["beta"],
                "beta_vae_beta": baseline["beta"],
...
def save_summary_plot(
    ...
    colors = {"beta_vae": "tab:blue", "beta_tc_vae": "tab:orange"}
    with plt.ioff():
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    for path in metadata:
        ...
        print(
            f"{record['model_name']} beta={record['beta']:g} "
            f"seed={record['seed']}: D={record['distortion']:.3f}, "
            f"R={record['rate']:.3f}, MIG={record['mig']:.3f}, "
    ...
    results = {
        ...
        "runs": records,
        "aggregate_by_model_and_beta": aggregate_records(records),
