...
This first lesson establishes the shared baseline for the next two scripts:
    ...
    - spectral normalization on generator and discriminator weights;
    - projection conditioning in the discriminator;
    - a class embedding concatenated to the generator input;
...
def main():
    ...
    discriminator = ProjectionSNDiscriminator(
        num_classes=NUM_CLASSES,
        base_channels=BASE_CHANNELS,
    ).to(device)
