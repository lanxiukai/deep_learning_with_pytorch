# `dl_utils` module reference

This is the detailed reference for the current source tree: conventions, the
directory inventory, and one entry per module. For installation, module
selection, and focused work, start with [README.md](README.md) and open only
the relevant source file or section below.
...
## Directory and module tree (alphabetical)
...
Dependencies: os, torch, numpy, torch.nn, torch.utils.data (Dataset), tqdm,
dl_utils.plot._backend (pyplot), dl_utils.data.images (load_rgb_image).
...
Without `DL_OUTPUT_ROOT`, GAN artifacts are rooted at `output/gan/`.
...
The VAE route is intentionally split by teaching concern. `vae.py` preserves
the 256x256 introductory VAE, while the 32x32 lessons use `vae_common.py` and
the focused modules below. The current route covers diagonal-Gaussian and
hierarchical VAEs, discrete tokenizers and priors, and VQGAN/KL-autoencoder
components; retired IAF, VQ-VAE-2, and VAE-GAN implementations are not exposed
by this package.
...
`GaussianVAE32` has an explicit diagonal-Gaussian particle API. The
importance helpers keep particle computation chunkable for evaluation without
reintroducing posterior-flow components.
...
Dependencies: __future__ (annotations), pathlib (Path), torch,
torch.nn.functional (as F), torch.utils.data (DataLoader), torchvision.utils
(save_image), dl_utils.data.factor_shapes (FactorShapes32),
dl_utils.training.checkpoints (save_model_weights),
dl_utils.vae.vae_common (diagonal_gaussian_kl_from_logvar),
dl_utils.vae.vae_hierarchy (ActiveUnitAccumulator, HierarchicalVAE32,
hierarchical_vae_loss, model_config).
Public entries (__all__): evaluate_hierarchy, make_factor_shape_loaders,
train_hierarchy, warmup_weight.
The loop provides FactorShapes loaders, KL warm-up, deterministic evaluation,
final model-weight saving, and reconstruction artifacts; lesson scripts retain
the model configuration and experiment budget.
...
The current discrete route uses one 8x8 token grid per image: `VQVAE32` has a
learned codebook, while `FSQAutoencoder32` uses finite scalar quantization.
...
These priors serve the VQ-VAE, FSQ, and VQGAN lessons.

### dl_utils/vae/perceptual_autoencoder.py

Dependencies: torch, torch (Tensor, nn), dl_utils.vae.quantization
(VectorQuantizer), dl_utils.vae.vae_common (reparameterize_logvar);
`lpips` and `torchvision.models` are imported lazily only when their pretrained
perceptual losses are constructed.
...
`build_perceptual_loss` selects learned LPIPS, pretrained VGG, or an explicitly
non-metric random-feature check; LPIPS and VGG construction can require model
weight downloads.
...
Dependencies: math, os, pathlib (Path), typing (Any), torch, torchvision,
dl_utils.gan.inference (generate_in_batches),
dl_utils.plot._backend (pyplot).
