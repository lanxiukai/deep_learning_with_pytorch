"""Temporary self-contained DDPM helpers for the diffusion lesson."""

# pyright: reportMissingImports=false, reportOptionalOperand=false, reportOperatorIssue=false, reportAttributeAccessIssue=false, reportArgumentType=false, reportGeneralTypeIssues=false

# noqa: SIZE_OK — temporary self-contained DDPM lesson dependency closure.
...
def clip(tensor, min_value=None, max_value=None):
    ...
    raise ValueError(
        "Tensor format is not valid is not valid - "
    ...
    def __init__(
        ...
        if beta_schedule == "linear":
            ...
            self.betas = cosine_beta_schedule(
                num_train_timesteps, beta_start=beta_start, beta_end=beta_end
            )
        else:
